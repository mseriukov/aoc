...
public final class Day: DayType {
    ...
    struct Hand {
        ...
        func weight(for part: Part) -> Int {
            let maxcards = part == ._1 ? cards : self.maxcards
            ...
            for (idx, card) in cards.reversed().enumerated() {
                let multiplier = part == ._1 ? card.strength : (card == ._J ? 1 : card.strength)
    ...
    func runFirst(_ input: String) -> String {
        ...
        hands.sort(by: { $0.weight(for: ._1) < $1.weight(for: ._1) })
    ...
    func runSecond(_ input: String) -> String {
        var hands = parseInput(input)
        hands.sort(by: { $0.weight(for: ._2) < $1.weight(for: ._2) })
